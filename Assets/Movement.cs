...
public class Movement : MonoBehaviour {
	...
	void Update ()
    {
        ...
        if (Input.GetKey(KeyCode.W))
            move_direction += transform.forward * movement_speed * Time.deltaTime;
        if (Input.GetKey(KeyCode.A))
            move_direction -= transform.right * movement_speed * Time.deltaTime;
        if (Input.GetKey(KeyCode.S))
            move_direction -= transform.forward * movement_speed * Time.deltaTime;
        if (Input.GetKey(KeyCode.D))
            move_direction += transform.right * movement_speed * Time.deltaTime;

        controller_.Move(move_direction);
